...
public class FilesTest {
    ...
    public void test() throws Exception {
        ...
        assertThat(slack.methods(ValidToken).filesUpload(r -> r.filename("name").channels(Arrays.asList("C123")).title("title"))
                .isOk(), is(true));
        assertThat(slack.methods(ValidToken).filesGetUploadURLExternal(r -> r.filename("name").length(100L))
                .isOk(), is(true));
    ...
    public void test_async() throws Exception {
        ...
        assertThat(slack.methodsAsync(ValidToken).filesUpload(r -> r.filename("name").channels(Arrays.asList("C123")).title("title"))
                .get().isOk(), is(true));
        assertThat(slack.methodsAsync(ValidToken).filesGetUploadURLExternal(r -> r.filename("name").length(100L))
                .get().isOk(), is(true));
